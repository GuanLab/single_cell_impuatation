...
def get_unet(the_lr=1e-1, num_class=2, num_channel=10, size=2048*25):
    ...
    convn = Conv1D(num_class, 1, activation='relu', padding=padding)(layer_up[-1])

    model = Model(inputs=[inputs], outputs=[convn])

    model.compile(optimizer=Adam(lr=the_lr,beta_1=0.9, beta_2=0.999,decay=1e-5), loss=rmse, metrics=['mean_squared_error'])
